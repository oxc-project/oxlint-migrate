    ...
    const getAtInternalLoc = (c, indexInComment) => {
      return {
        start: context.sourceCode.getLocFromIndex(c.range[0] + indexInComment),
          ...
          c.range[0] + indexInComment + atInternal.length
        ),
      };
    };
    ...
    const checkDeclaration = (node) => {
      ...
      for (let i = 0; i < blockComments.length; i++) {
        ...
        if (isJSDoc && seenJSDoc) {
          context.report({
            messageId: 'multipleJSDocError',
            node: c,
            loc: getJSDocStartLoc(c),
          });
        }
        ...
        if (!isJSDoc) {
          ...
          context.report({
            messageId: 'internalCommentOnUnexported',
            node: c,
            loc: getAtInternalLoc(c, indexInComment),
          });
        }
        // eslint-disable-next-line local/no-in-operator
          ...
          context.report({
            messageId: 'internalCommentOnPrivate',
            node: c,
            loc: getAtInternalLoc(c, indexInComment),
          });
        } else if (i !== last) {
