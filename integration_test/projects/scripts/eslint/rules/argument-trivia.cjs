...
module.exports = createRule({
  ...
  create(context) {
    ...
    const isSetOrAssert = (name) =>
      name.startsWith('set') || name.startsWith('assert');
    ...
    const isTrivia = (node) => {
      ...
      if (node.type === AST_NODE_TYPES.Literal) {
        ...
        return (
          node.value === null || node.value === true || node.value === false
        );
      }
    ...
    const checkArg = (node, i, getSignature) => {
      ...
      const hasNewLine = sourceCodeText
        .slice(commentRangeEnd, argRangeStart)
        .includes('\n');
