...
| `--type-aware [bool]`       | Include type aware rules (enabled by default). Use `--type-aware=false` to disable. These rules are supported with `oxlint --type-aware` and [oxlint-tsgolint](https://github.com/oxc-project/tsgolint) |
